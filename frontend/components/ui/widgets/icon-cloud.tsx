...
export function IconCloud({
  ...
  useEffect(() => {
    const items = icons ?? images ?? [];
    const newIcons: Icon[] = [];
    const numIcons = items.length || 24;
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    iconPositions.forEach((icon) => {
      ...
      const scale = (rotatedZ + radius * 1.5) / (radius * 2.2);
      const r = 22 * scale;
      const dx = x - screenX;
      const dy = y - screenY;
    ...
    if (canvas && ctx) {
      const animate = () => {
        ...
        iconPositions.forEach((icon, index) => {
          ...
          const scale = (rotatedZ + radius * 1.5) / (radius * 2.2);
          const opacity = Math.max(0.2, Math.min(1, (rotatedZ + radius) / (radius * 1.5)));
  ...
  }, [icons, images, iconPositions, isDragging, isPaused, mousePos, targetRotation, radius]);
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <canvas
        ...
        className="h-auto max-w-full cursor-grab active:cursor-grabbing"
