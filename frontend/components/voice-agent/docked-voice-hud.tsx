...
export function DockedVoiceHUD({
  ...
  const [orbOpen,      setOrbOpen]      = useState(true);  // full orb vs. tiny trigger
  const [controlsOpen, setControlsOpen] = useState(false); // radial + nav

  const containerRef = useRef<HTMLDivElement>(null);

  // Collapse orb when clicking outside the entire container
  useEffect(() => {
  ...
  return (
    <div
      ...
      ref={containerRef}
    >
      {/* ── Tiny trigger dot when orb is collapsed ── */}
      <AnimatePresence>
        {!orbOpen && (
          <motion.button
            key="trigger"
          ...
          >
            ...
            <AnimatePresence>
              {controlsOpen && RADIAL.map(({ id, angle }, idx) => {
                ...
                const y   = -(Math.sin(rad) * R); // positive = up in CSS
