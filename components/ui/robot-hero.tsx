...
function ResponsiveGroup({
  ...
  const s = Math.min(0.95, viewport.width / 3.8) * scale;
...
function texTWrap(texture: THREE.CanvasTexture) {
...
function RobotPrototype({
  ...
  useFrame((state, delta) => {
    ...
    const tx = windowPointerRef.current.active
      ? windowPointerRef.current.x
      : THREE.MathUtils.clamp(state.pointer.x, -1, 1);
    const ty = windowPointerRef.current.active
      ? windowPointerRef.current.y
      : THREE.MathUtils.clamp(state.pointer.y, -1, 1);

    // Free horizontal translation following the cursor
    const maxMoveX = Math.min(0.5, state.viewport.width * 0.12);
    ...
    const floatY = Math.sin(state.clock.getElapsedTime() * 2.0) * 0.035;
    ...
    const bodyTargetRotZ = -relativeX * 0.12;
